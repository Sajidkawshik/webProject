...
if($conn === false){
 ...
 $pql="SELECT * FROM du WHERE licse_no='$licse_no'"; 
...
/////////// data delete///////////////
 ...
 $sql="DELETE FROM du WHERE licse_no='$licse_no'";
 $result=mysql_query($sql);
 // if successfully updated. 
 if(mysqli_query($conn, $sql))
{
    echo "Records were deleted successfully.";
	header("Location:myqrcode/temporary.php?fname=$row[1]&email=$row[3]&dept=$row[4]&lname=$row[2]&con_add=$row[6]&blood=$row[8]&contact=$row[9]&license=$row[10]&car=$row[11]");
}
else
{
